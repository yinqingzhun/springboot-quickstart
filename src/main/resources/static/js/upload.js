...
var uploader = new plupload.Uploader(
		{
			...
			init : {
							...
							.each(
									...
									function(file) {
										document.getElementById('ossfile').innerHTML += '<div id="'
												...
												+ ')<b></b>'
												+ '<span style="padding: 0 10px;cursor: pointer;" onclick="this.parentNode.parentNode.removeChild(this.parentNode)">[x]</span>'
				...
				FileUploaded : function(up, file, info) {
					if (info.status == 200) {
						var url = "http://bj-res.oss-cn-beijing.aliyuncs.com/"
								+ get_uploaded_object_name(file.name);
						document.getElementById(file.id).getElementsByTagName(
								'b')[0].innerHTML = 'upload to oss success, <a href="'
								+ url + '">' + url + '</a>';
					} else {
						document.getElementById(file.id).getElementsByTagName(
